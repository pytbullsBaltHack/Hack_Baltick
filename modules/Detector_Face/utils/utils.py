...
def non_max_suppression(prediction, conf_thres=0.5, nms_thres=0.5):
    ...
    for image_i, pred in enumerate(prediction):
        ...
        det_max = []
        nms_style = 'AND'  # 'OR' (default), 'AND', 'MERGE' (experimental)
...
def read_list(input_path):
    ...
    :return: словарь строк
    """
    list_str=None
    if check_fpath(input_path):
        file_txt = open(input_path, 'rt')
        list_str = file_txt.read().rstrip('\n').split('\n')
    return list_str

def remove_files(dir_remove, mask_name):
    """
    Удаление файлов в директории по маске
    :param dir: directory for remove files
    :param mask_name:
    :return:
    """
    if os.path.isdir(dir_remove):
        list_files = os.listdir(dir_remove)
        for id_file in list_files:
            if id_file.find(mask_name) >= 0:
                os.remove(os.path.join(dir_remove, id_file))
                print(os.path.join(dir_remove, id_file))
                print('is deleted')

def post_predict(pred, pred_img_shape, orig_img_shape):
    """
    Post prediction
    :param pred:
...
def processing_frame(frame,img_size):
    """

    :param frame:
    :return:
    """
    img = [letterbox(frame, new_shape=img_size, interp=cv2.INTER_LINEAR)[0]]
    img = np.stack(img, 0)
